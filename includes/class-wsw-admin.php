...
class WSW_Admin {
	...
	public function enqueue_assets( $hook ) {
		...
		wp_add_inline_style(
			...
			.wsw-action-card{background:#fff;border:1px solid #c3c4c7;border-left:4px solid #2271b1;border-radius:3px;padding:14px 20px 4px;margin:28px 0 16px;max-width:780px;box-shadow:0 1px 1px rgba(0,0,0,0.04)}
			.wsw-action-card-header{display:flex;align-items:center;gap:8px;margin-bottom:4px}
			.wsw-action-card-header h3{margin:0}
			.wsw-action-card-header .dashicons{color:#2271b1}
			.wsw-action-card > .description{margin-top:0;color:#646970}
	...
	private function render_user_detail( $user_id ) {
		...
		<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="wsw-limits-form">
			...
			<p><?php submit_button( __( 'Save limits', 'wp-simple-wallet' ), 'secondary', 'submit', false ); ?></p>
		</form>

		<div class="wsw-action-card">
			<div class="wsw-action-card-header">
				<span class="dashicons dashicons-money-alt"></span>
				<h3><?php esc_html_e( 'Apply balance adjustment', 'wp-simple-wallet' ); ?></h3>
			</div>
			<p class="description"><?php esc_html_e( 'One-shot action: records a new transaction immediately when submitted. This is not a setting.', 'wp-simple-wallet' ); ?></p>
		...
		wp_safe_redirect(
			add_query_arg(
				array(
					...
					'tab'     => 'wallets',
					'user_id' => $user_id,
					'wsw_msg' => is_wp_error( $result ) ? rawurlencode( $result->get_error_message() ) : 'adjustment_applied',
				),
				admin_url( 'admin.php' )
			)
		);
